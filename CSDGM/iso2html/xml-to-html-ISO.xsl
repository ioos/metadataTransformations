...
<!--
======================================================================
	ISO 19115 and 19115-2 XML to HTML Stylesheet March 2013
	...
	Revisions:
	    ...
	    20130320 (JLM) Updated some element names to display correctly (gml namespace) and added a few attributes to the display. 
	...
	<xsl:template name="printAttributes">
		<xsl:choose>
			<xsl:when test="./@*">
					...
					<xsl:if test="./@exponent"><xsl:value-of select="concat('     exponent: ', @exponent)"/></xsl:if>
					<xsl:if test="./@gml:id"><xsl:value-of select="concat('id: ', @gml:id)"/></xsl:if>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	<!--
